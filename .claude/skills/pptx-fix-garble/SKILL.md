...
2. **既定: 対話セッションなら、ここで「対話式回答」節へ進む**(チャットで
   1件ずつ選択式に聞く)。以下の手順2〜3は、ユーザーが「後で答える」と
   言った場合と、非対話の自動実行の場合のみ。

2'. 各タスクを判断する(非対話時)。タスクごとに:
...
## 対話式回答(これが既定。要判断はまずチャットで聞く)

要判断が残ったら、**既定ではその場のチャットで選択式に回答を集める**。
HTMLレポートに退避するのは、ユーザーが「後で答える」「レポートで」と
言った場合と、対話できない実行環境(ウォッチャー等)の場合のみ:

1. タスクを4件ずつ処理する。各件について、まず `crops/<id>_source.png` を
   Readで表示して見せてから、AskUserQuestionで選択肢を提示:
   - 「予測を反映: <suggested>」(suggestionsがある場合、推奨として先頭に)
   - 「そのまま(今の文字で正しい)」
   - 「消す」
   - 「画像のまま残す」
   自由入力は Other で受ける(正しい文言をそのまま書いてもらう)
2. 回答を answers.json に集約し、手順4(適用)→手順5(レポート)を通常どおり実行
   (レポートは記録として必ず生成する。保留が残ればそのレポートで後から回答できる)
3. ✓ 確認: 回答を集めた件数+保留の件数 = N であること
