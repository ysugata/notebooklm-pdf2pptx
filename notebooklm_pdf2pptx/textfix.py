...
class TextRepairer:
    ...
    def propose(self, text: str) -> str | None:
        """確定基準に届かない場合の「最有力の推測」を返す(自動適用しない)。

        レポートの選択肢「予測を反映」用。フレーズ照合を類似0.45まで
        緩めて推測する。未知の崩れ字の意味的な推測はエージェント
        (Claude/Codex)が切り抜き画像を見て suggestions.json に書く方式
        (ローカルMLMは実測で効果が無く撤去した。docs/DESIGN.md参照)。
        """
        fixed, _ = self.apply(text)
        t, _ = self._snap_to_known_phrase(fixed, min_sim=0.45, strict=False)
        return t if t != text else None


    def _snap_to_known_phrase(self, text: str, min_sim: float = 0.7,
                              strict: bool = True) -> tuple[str, list[tuple[str, str]]]:
